<script setup lang="ts">
    ...
    // สร้างตัวแปรไว้ผูกกับฟอร์มและเก็บข้อมูล ภายใน ref คือตัวแปรเพื่อเก็บข้อมูล form
    const username = ref('')
    const password = ref('')
                ...
                `${SPRING_API}/authenticate/login`,
                {
                    ...
                    onResponse({ request, response, options }) {
                        ...
                        if(response.status == 401){
                            ...
                            // alert('Login Success')
                            // console.log('userdata ', response._data.data)

                            // บันทึก token ลง cookie
                            token.value = response._data.data.token
                            console.log("line84 token =",token)

                            // บันทึก user data ลง cookie
                            userData.value = response._data.data
                            console.log("line88 userData =",userData)
                            console.log("line88 userData =",userData.value["email"])
            ...
            <VCol class="hidden-sm-and-down fill-height" md="6" lg="6">
                ...
                >
                    <div class="text-center w-50 text-white mx-auto">
                        <h1 class="mb-4">Welcome back.x13:44</h1>
